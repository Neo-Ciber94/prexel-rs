...
impl <N: Add<N, Output = N>> BinaryFunction<N> for AddOperator{
    ...
    fn precedence(&self) -> Precedence { Precedence::LOW }
...
impl <N: Mul<N, Output = N>> BinaryFunction<N> for MulOperator{
    ...
    fn precedence(&self) -> Precedence { Precedence::MEDIUM }
...
impl <N: Neg<Output = N>> UnaryFunction<N> for UnaryMinus{
    ...
    fn notation(&self) -> Notation { Notation::Prefix }
...
impl<N: Add<N, Output = N> + Div<N, Output = N> + FromPrimitive + Clone> Function<N> for AvgFunction {
    fn name(&self) -> &str {
        "avg"
    }
    ...
    fn call(&self, args: &[N]) -> Result<N> {
        ...
        match sum {
            Some(n) => {
                Ok(n / N::from_usize(args.len()).unwrap())
            }
